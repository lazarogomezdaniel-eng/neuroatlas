...
import { Trash2, AlertTriangle, CheckCircle2, FileText, Download, Sparkles } from 'lucide-react';
      ...
      {stack.length === 0 ? (
        <div className="rounded-biotech border border-dashed border-surface-bright bg-surface-lowest/50 p-12 text-center">
          ...
          <p className="text-xs text-text-muted max-w-sm mx-auto">
            Explora la enciclopedia o la matriz de sustancias y haz clic en &quot;+ Añadir a Stack&quot; para evaluar sinergias y dosificaciones combinadas.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-3">
            <h3 className="text-xs font-mono font-bold text-text-muted uppercase tracking-wider">
              Compuestos Activos ({stack.length})
            </h3>
            <div className="space-y-3">
              {stack.map((s) => (
                ...
                >
                  ...
                  <button
                    onClick={() => onRemoveFromStack(s.id)}
